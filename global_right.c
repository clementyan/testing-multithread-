...
#pragma omp parallel default(none) shared(cpu_alloc,migration_counter,MatrixC,MatrixA,MatrixB) private(i,j)
{
	cpu_alloc[omp_get_thread_num()]= sched_getcpu();
	#pragma omp for
	for (i=0; i < 3000; i++)
	{
//#pragma omp parallel default(none) shared(cpu_alloc,migration_counter,MatrixC,MatrixA,MatrixB) private(j)
//{
//	cpu_alloc[omp_get_thread_num()]= sched_getcpu();
//	#pragma omp for
		for (j=0; j<3000; j++) 
